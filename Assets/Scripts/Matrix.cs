...
public class Matrix : MonoBehaviour
{
    ...
    void SaveImage()
    {
        ...
        for (int row = 0; row < rowCount; row++)
        {
            for (int col = 0; col < colCount; col++)
            {
                ...
                var color = cell.isAlive == true ? Color.white: Color.black;
    ...
    void Run()
    {
        // Percorre a matriz decidindo quem morre e quem vive
        for (int col = 0; col < colCount; col++)
        {
            for(int row = 0; row < rowCount; row++)
            {
                ...
                if (cell.isAlive && count < 2) // Qualquer célula viva com menos de dois vizinhos vivos morre de solidão.
                {
                    cell.nextStatus = false;
                }
                if (cell.isAlive && count > 3) // Qualquer célula viva com mais de três vizinhos vivos morre de superpopulação.
                {
                    cell.nextStatus = false;
                }
                if (count == 3) // Qualquer célula com exatamente três vizinhos vivos se torna uma célula viva.
                {
        ...
        // Executa o próximo estado em todas as células
        for (int col = 0;col < colCount; col++)
        {
            for (int row = 0; row < rowCount; row++)
            {
